...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_SESSION['user_id']) || empty($_SESSION['user_id'])) {
        ...
        if ($maxMembers < 2) {
            ...
            $query = "INSERT INTO groupInfo (groupID, groupName, numLimit, meetingTimes, groupDescription, dateCreated) 
                      VALUES (?, ?, ?, ?, ?, NOW())";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("isiss", $group_id, $groupName, $maxMembers, $meetingTimes, $description);
            ...
            if ($stmt->execute()) {
                ...
                $insert_member_query = "INSERT INTO isAmember (groupId, memberId, groupRole, membertype) 
                                         VALUES (?, ?, ?, ?)";
                $stmt_member = $conn->prepare($insert_member_query);
                $stmt_member->bind_param("iiss", $groupID, $userID, $groupRole, $memberType);
    ...
    <main class="main-content">
        <section class="section">
            ...
            <form action="creategroup.php" method="post" class="group-form">
                ...
                <input type="text" id="group-id" name="groupID" placeholder="Enter group ID" required>
                ...
                <input type="text" id="meeting-times" name="meetingTimes" placeholder="E.g., Mondays 3 PM" required>
